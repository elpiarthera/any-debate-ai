    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-lg">
            ...
            {plans.map((plan) => (
              <button
                className={cn(
                  ...
                  selectedPlan === plan.id ? "border-primary bg-primary/5" : "border-input hover:border-primary/50",
                )}
                key={plan.id}
              ...
              >
                ...
                <span className="text-center text-muted-foreground text-xs">{plan.description}</span>
              </button>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button type="button">Continue with {plans.find((p) => p.id === selectedPlan)?.name}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
